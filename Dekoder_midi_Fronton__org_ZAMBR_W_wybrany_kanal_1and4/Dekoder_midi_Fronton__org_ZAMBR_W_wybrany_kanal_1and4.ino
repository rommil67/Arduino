...
#define decoder_channel_ped 3 // PEDAL ; the selected MIDI channel on which the decoder works
#define decoder_channel_man_I 1
#define decoder_channel_coppel 3
...
void MyHandleNoteOn(byte channel, byte pitch, byte velocity) {
  if (channel == decoder_channel_ped) {
   switch (pitch) {
    case 67:
    ...
    if (channel == decoder_channel_man_I || channel == decoder_channel_coppel) {  
   switch (pitch) {
    case 59:
    digitalWrite (3, HIGH);
    break;
